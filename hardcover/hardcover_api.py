...
# Add parent directory and goodreads subdirectory to path for importing goodreads_genres
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(SCRIPT_DIR)
sys.path.append(parent_dir)
sys.path.append(os.path.join(parent_dir, "goodreads"))
...
def setup_logging(to_file_only=False):
    ...
    file_handler = logging.FileHandler(os.path.join(SCRIPT_DIR, "hardcover_crawler.log"), encoding="utf-8")
...
def worker(thread_idx, args, query, headers, end_offset, state, pbar):
    ...
    while not stop_event.is_set():
        ...
        with state['lock']:
            ...
            checkpoint_file = os.path.join(SCRIPT_DIR, 'hardcover_checkpoint.json')
            try:
...
def main():
    ...
    parser.add_argument("--output-json", default=os.path.join(SCRIPT_DIR, "hardcover_books.json"), help="Tên file JSON đầu ra")
    parser.add_argument("--output-csv", default=os.path.join(SCRIPT_DIR, "hardcover_books.csv"), help="Tên file CSV đầu ra")
