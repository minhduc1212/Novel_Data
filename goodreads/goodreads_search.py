...
data_path = os.path.join(SCRIPT_DIR, "goodreads_books.json")
if not os.path.exists(data_path):
    parent_data = os.path.join(os.path.dirname(SCRIPT_DIR), "goodreads_books.json")
...
def build_index(data_path, db_path, progress_callback=None):
    ...
    genres_path = os.path.join(SCRIPT_DIR, "goodreads_book_genres_initial.json")
    if not os.path.exists(genres_path):
        parent_genres = os.path.join(os.path.dirname(SCRIPT_DIR), "goodreads_book_genres_initial.json")
        if os.path.exists(parent_genres):
            genres_path = parent_genres
        else:
...
def main():
    ...
    parser.add_argument("--db-path", default=os.path.join(SCRIPT_DIR, "goodreads_books.db"), help="Path to the SQLite index database.")
