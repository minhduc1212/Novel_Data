...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(SCRIPT_DIR, "goodreads_scraper.log"), encoding="utf-8"),
...
def load_proxies_from_env(env_path=None) -> list:
    if env_path is None:
...
def main():
    ...
    isbn_file       = os.path.join(SCRIPT_DIR, "clean_novel_isbns.txt")
    if not os.path.exists(isbn_file):
        parent_isbn = os.path.join(os.path.dirname(SCRIPT_DIR), "clean_novel_isbns.txt")
        if os.path.exists(parent_isbn):
            isbn_file = parent_isbn
            
    json_file       = os.path.join(SCRIPT_DIR, "goodreads_books.json")
    csv_file        = os.path.join(SCRIPT_DIR, "goodreads_books.csv")
    checkpoint_file = os.path.join(SCRIPT_DIR, "goodreads_checkpoint.json")
