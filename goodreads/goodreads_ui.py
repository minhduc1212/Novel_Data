import sys
import os
# Add script directory to sys.path to enable loading goodreads_search when run from outside directory
...
class GoodreadsApp(customtkinter.CTk):
    def __init__(self):
        ...
        if not os.path.exists(self.data_path):
            local_json = os.path.join(os.path.dirname(os.path.abspath(__file__)), "goodreads_books.json")
            if os.path.exists(local_json):
                self.data_path = local_json
            elif os.path.exists("goodreads_books.json"):
                self.data_path = os.path.abspath("goodreads_books.json")
            
        self.db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "goodreads_books.db")
        if not os.path.exists(self.db_path) and os.path.exists("goodreads_books.db"):
            self.db_path = os.path.abspath("goodreads_books.db")
        self.current_page = 1
