...
# Check if the file is in the script directory, or parent directory, or fallback to the hardcoded path
goodreads_genres_path = os.path.join(SCRIPT_DIR, "goodreads_book_genres_initial.json")
if not os.path.exists(goodreads_genres_path):
    parent_path = os.path.join(os.path.dirname(SCRIPT_DIR), "goodreads_book_genres_initial.json")
    if os.path.exists(parent_path):
        goodreads_genres_path = parent_path
    else:
        goodreads_genres_path = "D:\\LT\\data\\goodreads_book_genres_initial.json"

db_path = os.path.join(SCRIPT_DIR, "goodreads_books.db")
if not os.path.exists(db_path):
    parent_db = os.path.join(os.path.dirname(SCRIPT_DIR), "goodreads_books.db")
